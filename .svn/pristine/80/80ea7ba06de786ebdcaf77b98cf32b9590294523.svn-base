$(function () {
    ...
    function queryBrandsList(queryStr,currentPage,pageSize) {
        var url;
        if(queryStr==""){
            url = httpUrl+"/adminRepair/list/"+currentPage+"/"+pageSize+"";
        }else{
            url = httpUrl+"/adminRepair/queryList/"+queryStr+"/"+currentPage+"/"+pageSize+"";
        }
            ...
            success:function (res) {
                ...
                $.each(res.data,function(i,v){
                    $brandlist.append(
                        ...
                        "<td class='cs_33' title='"+v.repairName+"'>"+v.repairName+"</td>"+
                ...
                $.each($options,function(){
                    $(this).click(function(){
                        if($(this).attr('class')=="delete"){
                            ...
                            window.location.href="repairfactory.html?id="+dataid+"&currentPage="+page;
                            //把当前页面带过去
                        }
                    })
                })
    ...
    function queryBrandsCount(queryStr) {
        var url;
        if(queryStr==""){
            url = httpUrl+"/adminRepair/getNum";
        }else{
            url = httpUrl+"/adminRepair/queryListNum/"+queryStr;
        }
        //上一次的页码 如果存在就清除掉
    ...
    function deleteBrand(id) {
        ...
        if(id=="" || id==null){
            ...
            $.ajax({
                type:"GET",
                url:httpUrl+"/adminRepair/delete/"+id+"",
                dataType:"json",
    ...
    $('#publishNew').click(function () {
        ...
        location.href="repairfactory.html?currentPage="+oldPage;
    });
